<template>
    <div id="add-task">
      <b-button class="add-btn" variant="outline-primary" size="sm" v-if="!show" v-on:click="show = !show">
      &plus; Add A New Task</b-button>
          ...
          <div v-if="show">
            ...
            header-text-variant="white">   
                <b-form>
                    ...
                    <b-input id="form-input-date" type="date"></b-input>
                    </b-input-group>

                    <label class="sr-only" for="form-input-decsription">Description</label>
                    <b-form-input id="form-input-description"
                    class="mb-2 mr-sm-2 mb-sm-0 input-field"
                    placeholder="Task Description" v-model="taskDescription"
                    ></b-form-input>

                    <label class="sr-only" for="form-input-flag">Flag: </label>
                    <b-input-group prepend="Flag: " class="mb-2 mr-sm-2 mb-sm-0 input-field">
                    <b-form-select id="form-input-flag" class="mb-2 mr-sm-2 mb-sm-0" :options = "flags">
                    </b-form-select></b-input-group>
                </b-form>

                <div id="btn">
                    <b-button variant="secondary" id="btn-cancel" v-on:click="show = !show">Cancel</b-button>
                    <b-button variant="primary" id="btn-create" @click="addTask">Add Task</b-button>
                </div>
            </b-card>
        </div>
      </transition>
    ...
    methods: {
        ...
        addTask(payload) {
            if (payload.text) {
                ...
                .catch((error) => {
                    console.log(error);
                    this.$parent.getAllTasks();
                });
            } else {
                alert('Enter some task details!')
            }   
        },
